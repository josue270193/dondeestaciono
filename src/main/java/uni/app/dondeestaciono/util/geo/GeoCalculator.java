...
public class GeoCalculator {
  ...
  /**
   * Calculates destination point, given starting point, distance in meters and bearing angle in
   * degrees, that is the angle starting from true north.
   *
   * @param startingPoint starting point
   * @param distanceInMeters distance in meters
   * @param bearingInDegrees the angle starting from true north in degrees
   * @return destination point
   */
  public static PointDto getDestinationPoint(
      PointDto startingPoint, double distanceInMeters, double bearingInDegrees) {
    PositionDto destinationPosition =
        getDestinationPosition(startingPoint.getPosition(), distanceInMeters, bearingInDegrees);
  ...
  /**
   * Calculates destination position, given starting position, distance in meters and bearing angle
   * in degrees, that is the angle starting from true north.
   *
   * @param startingPoint starting point
   * @param distanceInMeters distance in meters
   * @param bearingInDegrees the angle starting from true north in degrees
   * @return destination point
   */
  public static PositionDto getDestinationPosition(
      PositionDto startingPoint, double distanceInMeters, double bearingInDegrees) {
    ...
    double lat2Radian =
        Math.asin(
            Math.sin(latInRadians) * Math.cos(angularRadius)
                + Math.cos(latInRadians) * Math.sin(angularRadius) * Math.cos(bearing));

    double dlon =
        Math.atan2(
            Math.sin(bearing) * Math.sin(angularRadius) * Math.cos(latInRadians),
   ...
   *
   * @param positionFrom starting position
   * @param positionTo ending position
   * @return
   */
  public static double distance(PositionDto positionFrom, PositionDto positionTo) {
    ...
    return Math.acos(
            Math.sin(latRadianFrom) * Math.sin(latRadionTo)
                + Math.cos(latRadianFrom)
                    * Math.cos(latRadionTo)
                    * Math.cos(lonRadianFrom - lonRadionTo))
        * EARTH_RADIUS;
  }
